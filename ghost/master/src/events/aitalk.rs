...
pub static TALKS: Lazy<Vec<String>> = Lazy::new(|| {
  let mut v = Vec::new();
  v.extend_from_slice(&TALKS_SELF_INTRODUCE);
  v.extend_from_slice(&TALKS_LORE);
  v.extend_from_slice(&TALKS_PAST);
  v.extend_from_slice(&TALKS_ABSTRACT);
  v.extend_from_slice(&TALKS_WITH_YOU);
  v
});
...
pub static TALKS_SELF_INTRODUCE: Lazy<Vec<String>> = Lazy::new(|| {
  vec![
    "\
...
pub static TALKS_LORE: Lazy<Vec<String>> = Lazy::new(|| {
  vec![
    "\
...
pub static TALKS_PAST: Lazy<Vec<String>> = Lazy::new(|| {
  vec![
    "\
...
pub static TALKS_ABSTRACT: Lazy<Vec<String>> = Lazy::new(|| {
  vec![
    "\
    h1111209因果が巡ってきた。\\n\
    過去が現在を刈り取りに来た。\\n\
...
pub static TALKS_WITH_YOU: Lazy<Vec<String>> = Lazy::new(|| {
  vec![
    "\
    ...
    h1111201いえ、いいのよ。h1111209むしろ、考えなしにしゃべる人よりよほど好ましいわ。\\n\
    h1111209雄弁は銀、沈黙は金というもの。\\n\
    h1111204必要なときに口を開けるかが大事よね。\
    ".to_string(),
    ...
    ……ともかく、彼女の姿を何かに残しておきたくなって、半ば衝動的に描きはじめたのだ。\\n\
