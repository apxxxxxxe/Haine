...
static DIALOG_SEXIAL_AKIRE: Lazy<Vec<String>> = Lazy::new(|| {
  vec![
    ...
    "h1111304面白い顔。h11113010鏡で見せてあげたいわ。".to_string(),
    "h1111104悪戯がすぎるわよ。".to_string(),
    "h1111103はあ……h1111106何が楽しいんだか。".to_string(),
    "h1111204その熱意は買うけれど。……h11112010虚しくないの？".to_string(),
...
pub fn mouse_dialogs(info: String, vars: &mut GlobalVariables) -> Option<Vec<String>> {
  match info.as_str() {
    ...
    "0skirtup" => {
      ...
      if !vars.volatility.first_sexial_touch() && vars.volatility.ghost_up_time() < 30 {
        ...
        conbo_parts.push(vec![
          "h1111204……いいもの見たって顔してる。h11112010屈辱だわ。".to_string(),
          "h1111205……ああ、ひどい人。h11112010泣いてしまいそうだわ。".to_string(),
    ...
    "0shoulderdown" => Some(first_and_other(
      ...
      h1111204……それで？h11112010あなたは私をどうしたいのかしら。\
      "
        .to_string(),
      ],
    )),
...
fn bust_touch(vars: &mut GlobalVariables) -> Vec<String> {
  ...
  if !vars.volatility.first_sexial_touch() && vars.volatility.ghost_up_time() < 30 {
    ...
    zero_bust_touch.extend(vec![
      ...
      "h11112010気を引きたいだけなら、もっと賢い方法があると思うわ。".to_string(),
...
pub fn head_hit(vars: &mut GlobalVariables) -> Vec<String> {
  ...
  if !vars.flags().check(EventFlag::FirstHitTalkDone) && !is_aroused {
    ...
    h12223010ウフ、あなたのせいで思い出しちゃった。\\n\
    痛いってこういうものだったわね。\\n\
    h1222506アハハ、素敵だわ、とても。\\n\
    h12225011ねえ、もっとやってみて。\\n\
    嫌悪したから叩いたのでしょう？あなた。\\n\
    双方に得があるのよ、遠慮はいらないわ。\\n\
    さあ。h1322813さあ！\\n\
    "
    ...
    all_combo(&vec![
      ["h12214010ぐっ……", "h12214011痛っ……", "h1221714づっ……"]
        .iter()
        .map(|s| s.to_string())
        .collect(),
      vec!["\\n".to_string()],
      [
        "h1311204すてき。h13112011もっとして。",
        "h1111206ああ、星が舞っているわ。h1311215痛くて苦しくて、死んでしまいそう。",
        "h1111204ひどい。h11112010ひどいわ。\\nh1311513癖になってしまったらどうするの？",
      ]
      .iter()
      .map(|s| s.to_string())
      .collect(),
