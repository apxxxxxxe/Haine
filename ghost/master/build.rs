...
fn main() {
    ...
    if Path::new(bincode_path).exists() {
        println!("cargo:warning=Pre-compiled dictionary found: {}", bincode_path);
    ...
    dict.write(&mut output_file).expect("Failed to write bincode dictionary");
    
    println!("cargo:warning=✓ Dictionary pre-compilation completed: {}", bincode_path);
}
